...
def _layernorm_fwd_kernel(
    ...
    if N_COLS <= BLOCK_SIZE_N:
        ...
        for col_offset in range(0, N_COLS, BLOCK_SIZE_N):
            ...
            # Masked tail cols (cols_off >= N_COLS) carry garbage in x_centered here,
            # but w_chunk/b_chunk are 0 there and the store uses block_mask, so they
            # never affect results. Kept un-masked (unlike the single-tile branch) to
            # save an op; relies on the block_mask store to stay correct.
